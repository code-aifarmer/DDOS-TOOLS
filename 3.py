...
ds=[]
def running():
    tkMessageBox.showinfo("ATTENTION!!!", "The target is seted")
    global ds
    ds.append(entry1.get())
    ds.append(entry2.get())
#print(target)
useragents = [
...
if __name__ == "__main__":
    ...
    myWindow.title('DDOS TOOLS')

    myWindow.geometry('300x100')

    myWindow.resizable(width=False, height=True)

    Label(myWindow, text="input the target").grid(row=0)
    Label(myWindow, text="input the threads").grid(row=1)
    entry1 = Entry(myWindow)
    entry2 = Entry(myWindow)
    entry1.grid(row=0, column=1)
    entry2.grid(row=1, column=1)

    Button(myWindow, text='FIRE', command=myWindow.quit).grid(row=2, column=2, sticky=W, padx=5, pady=2)
    Button(myWindow, text='COMPLETE SET', command=running).grid(row=2, column=0, sticky=W, padx=5, pady=2)
    Button(myWindow, text='STOP', command=st).grid(row=2, column=1, sticky=W, padx=5, pady=2)
    myWindow.mainloop()
